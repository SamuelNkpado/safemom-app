...
void main() {
  ...
  blocTest<EmergencyBloc, EmergencyState>(
    ...
    build: () {
      when(
        () => repository.getActiveRequest(any()),
      ...
      when(
        () => repository.markPartnerNotified(any()),
      ).thenAnswer((_) async {});
      when(
        () => repository.watchRequest(any()),
      ).thenAnswer((_) => Stream.value(dispatchedRequest));
      return bloc;
    },
    act: (bloc) => bloc.add(
      const SosTriggered(
    ...
    expect: () => [
      ...
      predicate<EmergencyState>(
        (s) =>
            s.sosStatus == SosStatus.active &&
            s.request?.driverName == 'Joseph Kamau',
      ),
    ],
  );
  ...
  blocTest<EmergencyBloc, EmergencyState>(
    ...
    build: () {
      when(
        () => repository.getRequest(any()),
      ).thenAnswer((_) async => dispatchedRequest);
      when(() => repository.cancelRequest(any())).thenAnswer((_) async {});
      return bloc;
    },
    seed: () =>
        EmergencyState(sosStatus: SosStatus.active, request: dispatchedRequest),
